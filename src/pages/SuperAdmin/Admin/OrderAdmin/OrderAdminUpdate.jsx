...
function OrderAdminUpdate() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get(`http://localhost:8000/auth/order-admins-edit/${id}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      let orderAdmin = await axios.patch(`http://localhost:8000/auth/order-admins-edit/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      console.log("Response:", orderAdmin);
                  ...
                  <div className="card-body">
                    <form>
                      ...
                      <div className="mb-3">
                        ...
                        <input type="email" name='email' className="form-control" onChange={handleOnChange} value={userData.email}/>
                      </div>
                      <div className="mb-3">
                        <label className="form-label">Username</label>
                        <input type="tel" name='username' className="form-control" onChange={handleOnChange} value={userData.username}/>
                      </div>
                      <div className="mb-3">
                        <label className="form-label">Password</label>
                        <input type="text" name='password' className="form-control" onChange={handleOnChange} value={userData.password}/>
                      </div>
                    </form>
                  </div>
                </div>
